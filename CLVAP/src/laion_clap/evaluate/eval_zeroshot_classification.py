"""
Evaluate zero-shot classification performance on different checkpoints for the CLVAP model.
"""
...
def evaluate_zeroshot(model, data, start_epoch, args, writer):
    ...
    with torch.no_grad():
        ...
        # Tokenize texts
        text_inputs = tokenize(all_texts).to(device)
        ...
        logits_per_audio = (logit_scale_a * all_audio_features @ text_features.t()).detach().cpu()

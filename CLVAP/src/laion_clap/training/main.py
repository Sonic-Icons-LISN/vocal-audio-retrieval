...
def is_pretrained_params(n):
    return (
            n.startswith("text_branch") or
            n.startswith("vocal_branch") or
            n in ["positional_embedding", "text_projection", "vocal_projection"]
...
def main():
    ...
    if args.name is None:
        args.name = "-".join(
            [
                ...
                f"amodel_{args.amodel}",
                f"tmodel_{args.tmodel}",
                f"vmodel_{args.vmodel}",
                f"lr_{args.lr}",
    ...
    if args.precision == "fp16":
        logging.warning(
            ...
            "They may cause NaN loss and NaN gradients. "
            "FP16 and AMP support needs further verification and tuning."
        )
    ...
    model, model_cfg = create_model(
        ...
        pretrained_vocal=args.pretrained_vocal,
    ...
    # Freeze text encoder parameters if specified
    text_freeze_parameters = [
        p
        for n, p in named_parameters
        if 'text_branch' in n and p.requires_grad
    ]
    if args.freeze_text:
        logging.info("Freezing Text Encoder Parameters")
        for p in text_freeze_parameters:
            p.requires_grad = False

    # Freeze vocal encoder parameters if specified
    vocal_freeze_parameters = [
        p
        for n, p in named_parameters
        if 'vocal_branch' in n and p.requires_grad
    ]
    if args.freeze_vocal:
        logging.info("Freezing Vocal Encoder Parameters")
        for p in vocal_freeze_parameters:
            p.requires_grad = False
    ...
    # Set weight decay-related params to 0 if using Adam optimizer
    ...
    if args.wandb and is_master(args):
        if wandb is not None:
            ...
            # Configure wandb for your project
            wandb.init(
                entity="clvap",
                project="clvap",
    ...
    for epoch in range(start_epoch, args.epochs):
        # Freeze the text and vocal parameters after a certain epoch if specified
        if epoch == args.freeze_text_after:
            logging.info("Freezing Text Encoder Parameters from this epoch.")
            for p in text_freeze_parameters:
                p.requires_grad = False
        if epoch == args.freeze_vocal_after:
            logging.info("Freezing Vocal Encoder Parameters from this epoch.")
            for p in vocal_freeze_parameters:
                p.requires_grad = False
        ...
        if (
            ...
            if args.save_top_performance:
                ...
                ]  # Check all R@10 metrics (all datasets) and use it to update the ckpt
